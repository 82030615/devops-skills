...
description: Java项目业务功能只读分析技能。输入实体类名或表名，分析入口点、调用链路、实体关系、状态机，输出Mermaid UML Markdown报告。触发词：业务分析、分析业务、Java业务分析、分析实体、分析表
---
...
**⚠️ 输入验证**：
- 如果输入为空或无法识别 → 提示用户输入实体类名或表名
- 如果同时匹配类名和表名 → 优先按实体类名处理，并提示用户确认
